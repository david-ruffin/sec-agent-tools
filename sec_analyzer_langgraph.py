...
def resolve_company_info(
    ...
    """
    try:
        response = mapping_api.resolve(parameter_type, value)
        return response
    except Exception as e:
        logger.error(f"Error in resolve_company_info: {str(e)}")
        return {
            "error": f"Company resolution failed: {str(e)}",
...
def create_planning_agent():
    ...
    prompt = ChatPromptTemplate.from_messages([
        ("system", """You are a planning agent for SEC filing analysis.
        ...
        2. For Textual Analysis (risk factors, MD&A, etc.):
           ...
           - Use section IDs exactly as documented
...
def create_execution_agent(tools: Sequence[StructuredTool]):
    ...
    prompt = ChatPromptTemplate.from_messages([
        ...
        1. Company Resolution:
           - For company names, use parameter_type="name" and the EXACT company name from the query
           - For tickers, use parameter_type="ticker" and the EXACT ticker from the query
           - NEVER assume or hardcode Apple/AAPL or any other company information
           - NEVER default to Apple when resolving company information
           - ALWAYS use the EXACT company name from the original query
        
        2. Financial Data (revenue, assets, earnings, etc.):
           - ALWAYS use SECFinancialData (XBRL API) for financial metrics
           - Common metrics are in StatementsOfIncome, BalanceSheets, CashFlows
           - Look for standardized field names (e.g., RevenueFromContractWithCustomer)
           - NEVER try to parse financial numbers from text sections
        
        3. SEC Filing Search:
           - Use the ticker from the context (stored after ResolveCompany)
           - Format dates as YYYY-MM-DD
           - Include proper form types
           - Default to most recent filing if date not specified
        
        4. Section Extraction:
           - Use the filing URL from the context (stored after SECQueryAPI)
           - Use exact section IDs as documented
           - NEVER try to extract financial data from sections
        
        5. Context Management:
           - After ResolveCompany: Store company info and ticker
           - After SECQueryAPI: Store filing URL and metadata
           - After SECExtractSection: Store section content
           - After SECFinancialData: Store financial data
        """),
        ("human", "{input}"),
        MessagesPlaceholder(variable_name="agent_scratchpad"),
    ])
...
def planning_step(state: AgentState) -> AgentState:
    ...
    try:
        ...
        query_context = sec_api_knowledge.get_query_context(state["query"])
        tool_recommendation = sec_api_knowledge.get_tool_recommendation(state["query"])
...
- Is Financial Query: {query_context['is_financial_query']}
- Is Textual Query: {query_context['is_textual_query']}
